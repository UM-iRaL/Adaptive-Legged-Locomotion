...
class NMPCController {
 public:
  ...
  /**
   * @brief update param for SSI-MPC
   */
  void initParamRff(const Eigen::MatrixXd &alpha,
                   const Eigen::MatrixXd &omega, 
                   const Eigen::VectorXd &b){
    mynlp_->initParamRff(alpha, omega, b);              
  }

  void updateRffAlpha(const Eigen::MatrixXd &alpha) {
    mynlp_->updateRffAlpha(alpha);
  }

  /**
   * @brief update param for L1-MPC
   */
  void initParamL1(const Eigen::VectorXd &sigma){
    mynlp_->initParamL1(sigma);              
  }
  
  void updateL1Sigma(const Eigen::VectorXd &sigma) {
    mynlp_->updateL1Sigma(sigma);
  }
 ...
 private:
  ...
  /// rf variable
  bool enable_rf_nmpc_ = false;

  /// l1 variable
  bool enable_l1_nmpc_ = false;
};
