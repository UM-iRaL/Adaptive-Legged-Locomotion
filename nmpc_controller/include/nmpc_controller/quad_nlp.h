...
#include "nmpc_controller/gen/eval_g_go2_rf.h"
#include "nmpc_controller/gen/eval_hess_g_go2_rf.h"
#include "nmpc_controller/gen/eval_jac_g_go2_rf.h"

#include "nmpc_controller/gen/eval_g_go2_l1.h"
#include "nmpc_controller/gen/eval_hess_g_go2_l1.h"
#include "nmpc_controller/gen/eval_jac_g_go2_l1.h"
...
enum SystemID {
  ...
  COMPLEX_TO_COMPLEX,
  COMPLEX_TO_SIMPLE
}; // make sure num_sys_id_ is equal to the length of SystemID
...
class quadNLP : public TNLP {
 public:
  ...
  /// enum)
  static const int num_sys_id_ = 9;
...
// ************************  variables for rf  ***************************
  bool enable_rf_nlp_ = false;
	int size_rf_ = 0;
  int size_target_mask_ = 0;
  int size_input_mask_ = 0;
	Eigen::MatrixXd rff_alpha_;
	Eigen::MatrixXd rff_omega_;
	Eigen::VectorXd rff_b_;

	int size_param_alpha = 0;
  int size_param_b = 0;
  int size_param_omega = 0;

	int pk_size_rf = 54;
//  

// ************************  variables for l1  ***************************
  bool enable_l1_nlp_ = false;
	int size_sigma_ = 0;
	Eigen::VectorXd l1_sigma_;
...
//  
  ...
  // function for RF
	void initParamRff(const Eigen::MatrixXd &alpha,
							const Eigen::MatrixXd &omega, 
							const Eigen::VectorXd &b) {
		
		rff_alpha_.resize(alpha.rows(), alpha.cols());
		rff_alpha_ = alpha;

		rff_omega_.resize(omega.rows(), omega.cols());
		rff_omega_ = omega;

		rff_b_.resize(b.size());
		rff_b_ = b;

		size_rf_ = alpha.cols();
		size_target_mask_ = alpha.rows();
		size_input_mask_ = omega.cols();

		size_param_alpha = size_target_mask_ * size_rf_;
    size_param_b = size_rf_;
    size_param_omega = size_rf_ * size_input_mask_;

		pk_size_rf = 54 + size_param_alpha + size_param_b + size_param_omega;	

		std::cout << "****************************************************" << std::endl;
		std::cout << "**********      nlp param sizes      **************" << std::endl;
		std::cout << "****************************************************" << std::endl;
		std::cout << "  n_rf is:     " << size_rf_ << std::endl;
		std::cout << "  I_tar is:    " << size_target_mask_ << std::endl;
		std::cout << "  I_input is:  " << size_input_mask_  << std::endl;
		std::cout << "  pk_size_new: " << pk_size_rf  << std::endl;
		std::cout << "****************************************************" << std::endl;
			
	}

  inline void updateRffAlpha(const Eigen::MatrixXd &alpha) {
    rff_alpha_.resize(alpha.rows(), alpha.cols());
    rff_alpha_ = alpha;
  }

  // function for L1
	void initParamL1(const Eigen::VectorXd &sigma) {

		l1_sigma_.resize(sigma.size());
		l1_sigma_ = sigma;

    size_sigma_ = sigma.size();

		pk_size_l1_ = 54 + size_sigma_;	

		std::cout << "****************************************************" << std::endl;
		std::cout << "**********      nlp param sizes      **************" << std::endl;
		std::cout << "****************************************************" << std::endl;
		std::cout << "  sigma:       " << l1_sigma_.transpose() << std::endl;
		std::cout << "  sigma size:  " << size_sigma_ << std::endl;
		std::cout << "  pk_size_new: " << pk_size_l1_  << std::endl;
		std::cout << "****************************************************" << std::endl;
	}

  inline void updateL1Sigma(const Eigen::VectorXd &sigma) {
		l1_sigma_.resize(sigma.size());
    l1_sigma_ = sigma;
  }
