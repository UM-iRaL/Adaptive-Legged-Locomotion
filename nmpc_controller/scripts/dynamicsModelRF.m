function dynamicsModelRF(parameter)
...
%add variables for rff
param_alpha = SX.sym('param_alpha', [parameter.size_target_mask, parameter.n_rf]);
param_b = SX.sym('param_b', [parameter.n_rf, 1]);
param_omega = SX.sym('param_omega', [parameter.n_rf, parameter.n_z]);

% generate Z
u_normalized = J_u*u /100.0;
% u_normalized = u /100.0;
Z = [x0(4:end); u_normalized];
param_rf = (1 / sqrt(parameter.n_rf)) * cos(param_omega * Z + param_b);
...
% add residual to veclocity dynamics
EOM(7:(7+parameter.size_target_mask-1)) = EOM(7:(7+parameter.size_target_mask-1)) - dt * param_alpha * param_rf;
...
%START add for rff
param_alpha_vec = reshape(param_alpha, parameter.n_rf*parameter.size_target_mask, 1);
param_omega_vec = reshape(param_omega, parameter.n_rf*parameter.n_z, 1);
p = [dt; mu; feet_location; param_alpha_vec; param_omega_vec; param_b];
%END add for rff
