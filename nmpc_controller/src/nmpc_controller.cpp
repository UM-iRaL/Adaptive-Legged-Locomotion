...
NMPCController::NMPCController(ros::NodeHandle &nh, int robot_id) {
  ...
  switch (robot_id_) {
    ...
    case 3:
      robot_ns_ = "go2_rf";
      default_system = GO2_RF;
      enable_rf_nmpc_ = true;
      break;
    case 4:
      robot_ns_ = "go2_l1";
      default_system = GO2_L1;
      enable_l1_nmpc_ = true;
      break;
    default:
      robot_ns_ = "go2";
      default_system = GO2;
  ...
  std::cout << "*****************NMPC*****************" << std::endl;
  std::cout << "    robot id: " << robot_id_ << std::endl;
  std::cout << "    robot type: " << robot_ns_ << std::endl;
  std::cout << "    rf  ENABLE: " << (enable_rf_nmpc_ ? "True" : "False") << std::endl;
  std::cout << "    l1  ENABLE: " << (enable_l1_nmpc_ ? "True" : "False") << std::endl;
  std::cout << "**********************************" << std::endl;
  ...
  quadKD_ = std::make_shared<quad_utils::QuadKD>();
	...
	if(enable_rf_nmpc_){
		mynlp_ -> enable_rf_nlp_ = true;
		mynlp_ -> enable_l1_nlp_ = false;
	} else if (enable_l1_nmpc_){
		mynlp_ -> enable_rf_nlp_ = false;
		mynlp_ -> enable_l1_nlp_ = true;
	} else {
		mynlp_ -> enable_rf_nlp_ = false;
		mynlp_ -> enable_l1_nlp_ = false;    
  }
