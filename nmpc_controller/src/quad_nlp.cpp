...
Eigen::VectorXd quadNLP::eval_g_single_complex_fe(int i,
  ...
  // Load the params for this fe
	Eigen::VectorXd pk; // Declare pk outside the conditional block.

	if (enable_rf_nlp_){
		pk.resize(pk_size_rf);
		...
		int idx = 14;
		if (size_param_alpha > 0) {
			// Flatten alpha matrix column-wise and insert into pk
			Eigen::Map<Eigen::VectorXd> param_alpha_vec(rff_alpha_.data(), size_param_alpha);
			pk.segment(idx, size_param_alpha) = param_alpha_vec;
			idx += size_param_alpha;
		}

		if (size_param_omega > 0) {
			// Flatten omega matrix column-wise and insert into pk
			Eigen::Map<Eigen::VectorXd> param_omega_vec(rff_omega_.data(), size_param_omega);
			pk.segment(idx, size_param_omega) = param_omega_vec;
			idx += size_param_omega;
		}

		if (size_param_b > 0) {
			pk.segment(idx, size_param_b) = rff_b_;
			idx += size_param_b;
		}

		pk.segment(idx, 12) = foot_pos_world_.row(i + 1);
		idx += 12;
		pk.segment(idx, 12) = foot_vel_world_.row(i + 1);
		idx += 12;
		...
		if (use_terrain_constraint_ && sys_id_schedule_[i] == COMPLEX_TO_COMPLEX) {
			for (int j = 0; j < num_feet_; j++) {
				...
				pk(idx + j) = terrain_.atPosition("z_inpainted", 
													foot_pos.head<2>(), interp_type_);
				pk(idx + 4 + 3 * j) = terrain_.atPosition("normal_vectors_x",
													foot_pos.head<2>(), interp_type_);
				pk(idx + 5 + 3 * j) = terrain_.atPosition("normal_vectors_y",
													foot_pos.head<2>(), interp_type_);
				pk(idx + 6 + 3 * j) = terrain_.atPosition("normal_vectors_z",
													foot_pos.head<2>(), interp_type_);
			}
		} else {
			pk.segment(idx, 4).fill(-10);
			pk.segment(idx + 4, 12).fill(1);
		}
    assert(idx == pk_size_rf && "Index idx does not match pk_size. Parameter vector size mismatch.");
	} else if (enable_l1_nlp_){ 
    pk.resize(pk_size_l1_);
    ...
    if (size_sigma_ > 0) {
			pk.segment(idx, size_sigma_) = l1_sigma_;
			idx += size_sigma_;
		}
    ...
    assert(idx == pk_size_l1_ && "Index idx does not match pk_size. Parameter vector size mismatch.");
  } else {
		pk.resize(54);
		pk[0] = (i == 0) ? first_element_duration_ : dt_;
		pk[1] = mu_;
  ...
  for (int i = 0; i < N_ - 1; ++i) {
    ...
    // Load the params for this fe
		...
		if (enable_rf_nlp_){
			...
			if (use_terrain_constraint_ && sys_id_schedule_[i] == COMPLEX_TO_COMPLEX) {
				for (int j = 0; j < num_feet_; j++) {
					Eigen::Vector3d foot_pos =  get_primal_foot_state_var(w, i + 1).segment(3 * j, 3);
  ...
  if (values == NULL) {
    ...
    for (int i = 0; i < N_ - 1; ++i) {
      ...
      if (enable_rf_nlp_){
        ...
        pk[1] = mu_;
        pk.segment(2, 12) = foot_pos_body_.row(i + 1);
        // Set param_rff_values if size_param_rff_ > 0
        int idx = 14;
        if (size_param_alpha > 0) {
          // Flatten alpha matrix column-wise and insert into pk
          Eigen::Map<Eigen::VectorXd> param_alpha_vec(rff_alpha_.data(), size_param_alpha);
